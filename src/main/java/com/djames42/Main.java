...
import com.djames42.services.Meter;
...
public class Main {
    public static void main(String[] args) {
        ...
        char meterBar='-';
        if ( System.getProperty("output") != null && System.getProperty("output").equalsIgnoreCase("true") ) output=true;
        ...
        if ( myQuota.getRatio() > Float.valueOf(properties.getProperties("imap_quota"))) {
            imap.IMAPSendWarning(myQuota);
            meterBar = '*';
        }
        if (output) {
            System.out.printf("Speakeasy Quota: %,d of %,d (%.2f%%)\n", myQuota.getUsage(), myQuota.getLimit(), myQuota.getRatio());
            System.out.println(Meter.meter(myQuota.getRatio(),meterBar));
        }
    }
}
